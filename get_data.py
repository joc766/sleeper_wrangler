...
def load_players_data(conn, limit_to_existing=False):
    """
    ...
    try:
        ...
        if limit_to_existing:
            ...
            cursor.execute("SELECT DISTINCT PlayerID FROM DraftPick WHERE PlayerID IS NOT NULL")
            ...
            try:
                cursor.execute("SELECT DISTINCT PlayerID FROM MatchupRosterPlayer WHERE PlayerID IS NOT NULL")
                roster_players = cursor.fetchall()
                existing_player_ids.update([row[0] for row in roster_players])
            except:
                pass  # Table might not exist yet
            
            print(f"Found {len(existing_player_ids)} existing player references in database")
        ...
        if skipped_not_existing > 0:
            print(f"Skipped {skipped_not_existing} players not referenced in existing data")
